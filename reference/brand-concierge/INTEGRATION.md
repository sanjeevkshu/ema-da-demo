...
| 9 | Go live on `.aem.live`, then production | All | Steps 1–8 |
...
| `pulse-bc-prod` | production | the production domain | `prod` |
...
1. In the **development** sandbox, open Brand Concierge → create a concierge from **`https://main--ema-da-demo--sanjeevkshu.aem.live/`**.
...
2. **Watch the first crawl status.** This also answers the open question of whether `.aem.live` can be crawled.
   - **Success:** done. The site stays hidden from public search engines, because the platform's `noindex` header and `robots.txt` don't depend on Brand Concierge.
   - **Partial success, or fails:** use **Fix Issues** to download the error list. If it's a robots block, use one of the options in `README.md` §7: ask Adobe to allowlist the crawler, or set up a pre-launch domain whose `robots.txt` admits only the Brand Concierge crawler.
...
2. In `scripts/brand-concierge.js`, fill in `CONFIG`:
   ...
   ```
   - Leave `prod` empty until the production domain exists. Production then simply doesn't load the concierge.
...
- the production domain, at go-live
...
4. **Production domain, later:**
   - add the `prod` datastream (production sandbox) to `CONFIG`
   - add the domain to the surface rules
   - switch the sitemap `origin` (`reference/site-config/sitemap.yaml`)
   - set a `robots.txt` that allows crawling
   - point the Brand Concierge knowledge source at the production sitemap
